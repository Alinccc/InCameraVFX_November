	...
	methods: {
      ...
      if (!this.$slots.default) return;
			...
			items.forEach((propData, index) => {
				...
				propData.left = this.linear ? 0 : -1 * (size / 2 + Math.cos(angles[index]) * radius - itemSize / 2); // -1 to have the items in the right order
				propData.top = this.linear ? index * 50 + 80 : size / 2 - Math.sin(angles[index]) * radius - itemSize / 2;
