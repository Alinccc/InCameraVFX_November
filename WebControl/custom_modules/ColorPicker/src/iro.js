...
(function (global, factory) {
  ...
  */
  ...
  var prototypeAccessors = { hsv: { configurable: true },rgb: { configurable: true },hsl: { configurable: true },rgbString: { configurable: true },hexString: { configurable: true },hslString: { configurable: true },  alpha: { configurable: true } };
  /**
    * @desc mix two colors
  ...
  */
  ...
  prototypeAccessors.alpha.set = function(newValue) {
    const hsv = this.hsv;
    hsv.a = newValue;
    this.hsv = hsv;
  }
  ...
  prototypeAccessors.hsv.set = function (newValue) {
    ...
    if (this._onChange) {
      ...
      let newAlpha = this._value.a != null ? this._value.a : 100;
  ...
  */
  ...
  color.prototype.set = function set (value) {
    ...
    // Hacky workaround to get a default value for the alpha channel
    if (this._value.a === null || this._value.a === undefined) this._value.a = 100
  };
  /**
  ...
  */
  ...
  var slider = function slider(svg, opts) {
    ...
    let squareHeight = h / 3;
    var firstCheckerRow = baseGroup.insert("g");
    let nbSquareX = Math.ceil(w / squareHeight);
    let nbSquareY = Math.ceil(h / squareHeight);
    let invert = false;

    firstCheckerRow.insert("rect", {
      rx: radius,
      ry: radius,
      x: x - borderWidth / 2,
      y: y - borderWidth / 2,
    ...
    if (opts.sliderType === "a") {
      for (let j = 0; j < nbSquareY; j++) {
        for (let i = 0; i < nbSquareX - 1; i++) {
          // Remove corner squares
          if (i === 0 && j === 0) continue;
          if (j === 0 && i === nbSquareX - 1 || j === 0 && i === nbSquareX - 2) continue;
          if (j === nbSquareY - 1 && i === 0) continue
          if (j === nbSquareY - 1 && i === nbSquareX - 1) continue
          if (j === nbSquareY - 1 && i === nbSquareX - 2) continue
          
          let color;
          if (!invert) {
            color = i % 2 === 0 ? "#cecece" : "#eeeeee"
          } else {
            color = i % 2 !== 0 ? "#cecece" : "#eeeeee"
          }
          firstCheckerRow.insert("rect", {
            x: x - borderWidth / 2 + i * (squareHeight),
            y: y - borderWidth / 2 + j * (squareHeight),
            width: squareHeight,
            height: squareHeight,
            fill: color
          });
    ...
    rect.setGradient("fill", svg.gradient("linear", {
      0: {
        color: opts.sliderType == "v" ? "black" : "white",
        opacity: opts.sliderType === "v" ? 1 : 0
      },
      90: {
        color: "green",
        opacity: 1
  ...
  */
  ...
  slider.prototype.update = function update (color$$1, changes) {
    ...
    if (opts.sliderType == "v") {
      ...
      if (hsv.a === undefined || hsv.a === null) {
        return;
      }
      let percent = hsv.a / 100;
      this.marker.move(range.min + percent * range.w, opts.y + opts.h / 2);
    }
  };
  /**
  ...
  */
  ...
  slider.prototype.input = function input (x) {
    var opts = this._opts;
    var range = opts.range;
    var dist = Math.max(Math.min(x, range.max), range.min) - range.min;
    const output = {};
    const val = Math.round(100 / range.w * dist);
    if (this._opts.sliderType === "v") {
      output["v"] = val;
    } else if (this._opts.sliderType === "a") {
      output["a"] = val;
    }
  ...
  */
  ...
  function whenReady(callback) {
    if (document.readyState == READYSTATE_COMPLETE) {
      ...
      listen(document, [EVENT_READYSTATE_CHANGE], function stateChange() {
        if (document.readyState == READYSTATE_COMPLETE) {
          callback();
  ...
  */
  ...
  colorPicker.prototype._mount = function _mount (el, opts) {
    ...
    Object.keys(opts.events).forEach(key => {
      let callbacks = opts.events[key];
      if (Array.isArray(callbacks)) {
        callbacks.forEach(cb => this.on(key, cb))
      } else {
    ...
    this.el = el;
    const realHeight = opts.displayAlpha ? height + sliderHeight + sliderMargin : height;
    this.svg = new svgRoot(el, width, realHeight, opts.display);

    const __slider = new slider(this.svg, {
      sliderType: "v",
    ...
    let alphaSlider;
    if (opts.displayAlpha) {
      alphaSlider = new slider(this.svg, {
        sliderType: "a",
        x: leftMargin + borderWidth,
        y: bodyWidth + sliderMargin + sliderHeight + sliderMargin,
        w: bodyWidth - borderWidth * 2,
        h: sliderHeight - borderWidth * 2,
        r: sliderHeight / 2 - borderWidth,
        marker: marker,
    ...
    this.ui = [this.wheel, __slider]; // Create an iroStyleSheet for this colorWheel's CSS overrides

    if (opts.displayAlpha) {
      this.ui.push(alphaSlider);
    }
